...
# ВАЖНО: Настройка matplotlib ПЕРЕД импортом
os.environ['MPLBACKEND'] = 'Agg'
os.environ['PYTHONUNBUFFERED'] = '1'
sys.stdout.flush()

# Теперь импортируем matplotlib
...
print("Запускаем фоновую задачу...", flush=True)
thread = threading.Thread(target=background_logger, daemon=True)
thread.start()
time.sleep(2)
print("Фоновая задача запущена!", flush=True)
