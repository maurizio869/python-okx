#!/usr/bin/env python3
# price_jump_eval_OneCFocalL.py
# Last modified (MSK): 2025-09-01 15:10 — правка номер 1
# Changes:
# - Created new eval script compatible with OneCFocalL models
# - Uses 7 features, no normalization, same preprocessing as training

"""
Загружает модель из OneCFocalL и проверяет на candles_eval.json.
Сохраняет результаты в viz_data.npz для визуализации.
"""
...
MODEL_PATH = Path("lstm_jump.pt")       # обученная модель (переименованная из lstm_jump_PRAUC.pt)
...
class EvalDataset(Dataset):
    def __init__(self, df: pd.DataFrame):
        ...
        for i in range(SEQ_LEN, len(self.closes) - PRED_WINDOW):
            ...
            # Вычисляем метку (для оценки, не для обучения)
...
if not MODEL_PATH.exists():
    ...
    print("   - lstm_jump_PRAUC.pt (лучшая по PR_AUC)")
    print("   - lstm_jump_pnl.pt (лучшая по PnL)")
    print("   - lstm_jump_valacc.pt (лучшая по точности)")
    exit(1)
...
np.savez_compressed(
    ...
    jump_threshold=np.float32(JUMP_THRESHOLD),
    # Комиссии
...
print(f"  Используйте price_jump_visualize.py для визуализации")