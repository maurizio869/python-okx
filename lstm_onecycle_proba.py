...
# Last modified (MSK): 2025-08-22 21:42
...
- стандартизация по всему набору окон (StandardScaler)
"""
...
# Константы (взяты из актуального onecycle)
...
class CandleDataset(Dataset):
    def __init__(self, df: pd.DataFrame):
        ...
        raw_windows = []   # список окон формы (SEQ_LEN, 5) — [o,h,l,c,vol]
        labels      = []
        for i in range(SEQ_LEN, len(df) - PRED_WINDOW):
            # метка по максимуму close в будущем окне
            ...
            window = np.concatenate([prices_rel, vol_rel], axis=1)  # (SEQ_LEN, 5)
            raw_windows.append(window.astype(np.float32))

        # стандартизация по всем окнам
        if len(raw_windows) == 0:
            self.scaler = StandardScaler()
            self.samples = []
        else:
            all_rows = np.vstack(raw_windows)  # (N*SEQ_LEN, 5)
    ...
    def __getitem__(self, idx: int):
        x, y = self.samples[idx]             # x: (SEQ_LEN, 5)
        x = torch.from_numpy(x.T.astype(np.float32))  # (5, SEQ_LEN)
        return x, int(y)
...
for e in range(1, EPOCHS+1):
    ...
    # перебор порога на каждой 5-й эпохе (и на первой)
...
# Пороговый свип по PnL (валидация)
