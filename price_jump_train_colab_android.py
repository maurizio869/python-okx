# price_jump_train_colab_android.py
"""Версия для Pydroid3 на Android с файловым логированием
Обучает LSTM, метка = 1 если максимум Close за следующие 5 мин ≥ Open + 0.35%
"""
...
# Функция логирования для Android
def log(message):
    ...
    # Выводим в файл
    try:
        with open("/storage/emulated/0/pydroid_log.txt", "a", encoding="utf-8") as f:
            f.write(log_message + "\n")
            f.flush()
    except:
        pass
...
log(f"Загружено {len(df)} свечей")

ds = CandleDataset(df)
log(f"Создано {len(ds)} сэмплов")
log(f"Меток 1: {sum(1 for _, y in ds.samples if y == 1)}")
log(f"Меток 0: {sum(1 for _, y in ds.samples if y == 0)}")
...
for e in range(1, EPOCHS+1):
    ...
    log(f'Epoch {e}/{EPOCHS} loss {tot/len(train_ds):.4f} val_acc {corr/tot_s:.3f}')

MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
torch.save({"model_state":model.state_dict(),"scaler":ds.scaler}, MODEL_PATH)
log(f"✓ Модель сохранена в {MODEL_PATH.resolve()}")
