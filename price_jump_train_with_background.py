# price_jump_train_with_background.py
"""Версия с фоновым логированием для Pydroid3
"""
...
def load_dataframe(path: Path) -> pd.DataFrame:
    print(f"Загружаем файл {path}")  # Ваш print() здесь
...
class CandleDataset(Dataset):
    def __init__(self, df: pd.DataFrame):
        print("Создаём датасет...")  # Ваш print() здесь
...
print("=== НАЧАЛО ОБУЧЕНИЯ ===")  # Ваш print() здесь
print(f"Устройство: {DEVICE}")  # Ваш print() здесь

df = load_dataframe(TRAIN_JSON)
print(f"Загружено {len(df)} свечей")  # Ваш print() здесь

ds = CandleDataset(df)
print(f"Создано {len(ds)} сэмплов")  # Ваш print() здесь
print(f"Меток 1: {sum(1 for _, y in ds.samples if y == 1)}")  # Ваш print() здесь
print(f"Меток 0: {sum(1 for _, y in ds.samples if y == 0)}")  # Ваш print() здесь
...
print("Создаём модель...")  # Ваш print() здесь
model = LSTMClassifier().to(DEVICE)
opt   = torch.optim.Adam(model.parameters(), LR)
lossf = nn.CrossEntropyLoss()

print("Начинаем обучение...")  # Ваш print() здесь
for e in range(1, EPOCHS+1):
    print(f"Эпоха {e}/{EPOCHS}")  # Ваш print() здесь
    model.train(); tot=0
    for x,y in tl:
    ...
    print(f'Epoch {e}/{EPOCHS} loss {tot/len(train_ds):.4f} val_acc {corr/tot_s:.3f}')  # Ваш print() здесь

MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
torch.save({"model_state":model.state_dict(),"scaler":ds.scaler}, MODEL_PATH)
print(f"✓ Модель сохранена в {MODEL_PATH.resolve()}")  # Ваш print() здесь
print("=== ОБУЧЕНИЕ ЗАВЕРШЕНО ===")  # Ваш print() здесь